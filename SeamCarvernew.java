...
public class SeamCarvernew {
    private int width;
    private int height;
    private Picture pictureCopy;

    // create a seam carver object based on the given picture
    public SeamCarvernew(Picture picture) {
        if (picture == null) {
    ...
    public double energy(int x, int y) {
        ...
        up = pictureCopy.getARGB(x, y - 1);
        down = pictureCopy.getARGB(x, y + 1);
        left = pictureCopy.getARGB(x - 1, y);
        right = pictureCopy.getARGB(x + 1, y);
    ...
    public void removeVerticalSeam(int[] seam) {
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width - 1; col++) {
                ...
                if (col < seam[row]) {
                    tmpPicture.setARGB(col, row, pictureCopy.getARGB(col, row));
                }
                else {
                    tmpPicture.setARGB(col, row, pictureCopy.getARGB(col + 1, row));
                }
            }
        }
    ...
    private void transpose() {
        ...
        for (int row = 0; row < width; row++) {
            for (int col = 0; col < height; col++) {
                tmpPicture.setARGB(col, row, pictureCopy.getARGB(row, col));
            }
        }
        pictureCopy = tmpPicture;
        int tmp = height;
    ...
    private void validateSeam(int[] seam) {
        for (int i = 0; i < seam.length - 1; i++) {
            if (Math.abs(seam[i] - seam[i + 1]) > 1) {
                throw new IllegalArgumentException(
                        "two adjacent entries differ by more than 1 in seam\n");
            }
        }
    }
